...
class U(Cartesian):
    ...
    def cyclic(self, a,  mod: bool = True, length: int = 10) -> list[int]:
        """ 
        Generate the cyclic subgroup
        Args:
            a: generator of the cyclic subgroup
            mod: if True, return elements modulo n
            length: number of elements to generate
        Returns:
            list of elements in the cyclic subgroup
        """
        if a not in self.group():
            raise Exception("element <a> not in unit group.")
        power_group = [pow(a, k) for k in range(length)]
        if not mod:
            return list(power_group)
        else:
            return [pow(a, k, self.n) for k in range(self.order())]
    ...
    def generators(self) -> list[int]:
        ...
        out = []
        for g in self.group():
            generated = {pow(g, k, self.n) for k in range(self.order())}
